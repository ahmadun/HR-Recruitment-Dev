...
export const getJobs = async (req, res) => {
  try {
    ...
    const [results, metadata] = await db.query(
      `SELECT id, position, section, qualification, estimated_joining, timeline, application_open, application_close, about_company, status, createdAt, updatedAt FROM jobs WHERE status=1 and position like '%${search}%' or status=0 and position like '%${search}%'`
    );
